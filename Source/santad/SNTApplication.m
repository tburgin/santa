...
- (instancetype)init {
  self = [super init];
  if (self) {
    // TODO(bur/rah): Can santad detect if it is running as a system extension?
    NSProcessInfo *processInfo = [NSProcessInfo processInfo];
    if ([processInfo isOperatingSystemAtLeastVersion:(NSOperatingSystemVersion){10, 15, 0}]) {
      LOGD(@"Using Endpoint Security");
    ...
    if ([_eventProvider isKindOfClass:[SNTDriverManager class]]) {
      [((SNTDriverManager*)_eventProvider) fileModificationPrefixFilterAdd:@[ @"/.", @"/dev/" ]];

      // TODO(bur): Add KVO handling for fileChangesPrefixFilters.
      NSArray *filters = [configurator fileChangesPrefixFilters];
      [((SNTDriverManager*)_eventProvider) fileModificationPrefixFilterAdd:filters];
    }
    ...
    SNTDaemonControlController *dc =
        [[SNTDaemonControlController alloc] initWithDriverManager:_eventProvider
    ...
    _compilerController = [[SNTCompilerController alloc] initWithDriverManager:_eventProvider
                                                                      eventLog:_eventLog];
